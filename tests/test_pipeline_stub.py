...
def test_git_transaction_rollback(imported_ctx):
    ctx = imported_ctx
    guide_rel = ("01 Scriptures/Study Guides/Book of Mormon/1 Nephi/"
    ...
    (ctx.vault / guide_rel).write_text(baseline + "\nGARBAGE OUTSIDE MARKERS",
                                       encoding="utf-8")
    (ctx.vault / "70 AI Study Guides" / "junk.md").write_text("junk", encoding="utf-8")
    gitops.hard_restore(ctx)
    assert read_text(ctx.vault / guide_rel) == baseline
    assert not (ctx.vault / "70 AI Study Guides" / "junk.md").exists()
