...
def test_import_and_generation(imported_ctx):
    ...
    p = ctx.vault / "01 Scriptures/Canonical/Book of Mormon/1 Nephi/1 Nephi 3.md"
    assert p.exists()
    text = read_text(p)
    ...
    assert (ctx.vault / "01 Scriptures/Study Guides/Book of Mormon/1 Nephi/"
...
def test_immutability_guard_and_restore(imported_ctx):
    ctx = imported_ctx
    p = ctx.vault / "01 Scriptures/Canonical/Book of Mormon/1 Nephi/1 Nephi 1.md"
...
def test_validation_catches_bad_links(imported_ctx):
    ctx = imported_ctx
    bad = ctx.vault / "70 AI Study Guides" / "Bad Note.md"
    bad.write_text("# Bad\n\nSee [[Alma 36#^alma-36-999]] and [[No Such Note XYZ]].\n",
                   encoding="utf-8")
    from scripturegraph.validation import Report, check_files
    report = Report()
    check_files(ctx, report, paths=["70 AI Study Guides/Bad Note.md"])
