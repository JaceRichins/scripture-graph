---
ownership: system
mutable: ai
content_type: sec-source
sg-id: secsource:church-history-matters
source-tier: A
approval: APPROVED
---

# Church History Matters

**podcast** · Quality tier **A** · Status **APPROVED**
Hosts/creators: Scott Woodward, Casey Griffiths
Institution: Scripture Central
Homepage: https://doctrineandcovenantscentral.org/church-history-matters-podcast/
Perspective label: `lds_academic` (tracked separately from quality — §8)

> Historians engaging difficult Church-history questions with primary documents; facts distinguished from interpretation.

## Expertise domains

- Church history
- Restoration history
- difficult historical questions
- primary sources
- Joseph Smith Papers

## Known strengths

- engages hard questions directly
- primary-document driven
- distinguishes fact from interpretation

## Known limitations

- hosted perspective is faithful-academic; check critical literature for counterpoints on contested topics
