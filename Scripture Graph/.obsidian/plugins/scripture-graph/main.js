...
var import_obsidian4, COLORS, COLOR_HEX, MARK_BG, NoteModal, AnnotationService, NotesPopover;
var init_annotations = __esm({
  "src/social/annotations.ts"() {
    ...
    NoteModal = class extends import_obsidian4.Modal {
...
${text}` : text;
    ...
    NotesPopover = class extends import_obsidian4.Modal {
        ...
        if (isMine) {
          ...
          share.onclick = (e) => {
            this.svc.visibilityMenu((vis, gid, label) => {
              ...
              new import_obsidian4.Notice(`Now visible to: ${label}`);
              this.close();
            }).showAtMouseEvent(e);
          };
          ...
          del.onclick = async () => {
            ...
            try {
              ...
              new import_obsidian4.Notice("Deleted");
            } catch (e) {
              new import_obsidian4.Notice(`Delete failed: ${e.message}`);
            }
            this.close();
          };
        }
...
async function bookText(app, bookName, abbr) {
  ...
  if (!(dest instanceof import_obsidian6.TFile)) return null;
...
var import_obsidian6, TRANSLATIONS, fileCache, TranslationsModal;
var init_translations = __esm({
  "src/study/translations.ts"() {
    ...
    TranslationsModal = class extends import_obsidian6.Modal {
...
async function snippetFor(app, conn, needle) {
  ...
  if (!(f instanceof import_obsidian5.TFile)) return null;
  try {
...
var ConnectionsModal = class _ConnectionsModal extends import_obsidian5.Modal {
  ...
  for (const c of COLORS) {
    menu.addItem((i) => i.setTitle(`Highlight ${c}`).setIcon("highlighter").onClick((e) => {
      svc.visibilityMenu((vis, gid, label) => {
        ...
        new import_obsidian8.Notice(`Highlighted \u2014 ${label}`);
      }).showAtMouseEvent(e);
    }));
  }
  menu.addSeparator();
  menu.addItem((i) => i.setTitle("Add note\u2026").setIcon("pencil").onClick(() => {
    new NoteModal(s, hit.verseId, (text) => {
      svc.visibilityMenu((vis, gid, label) => {
        ...
        new import_obsidian8.Notice(`Note saved \u2014 ${label}`);
...
var WelcomeModal = class extends import_obsidian9.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian9.Setting(contentEl).setName("Your name").addText((t) => t.setPlaceholder("e.g. Mom").onChange((v) => name = v));
    new import_obsidian9.Setting(contentEl).setName("Invite code").setDesc("From the family member who runs Scripture Graph").addText((t) => t.setPlaceholder("XXXX-XXXX-XXXX").onChange((v) => invite = v));
    new import_obsidian9.Setting(contentEl).setName("This device").addText((t) => t.setValue(device).onChange((v) => device = v || "My device"));
    new import_obsidian9.Setting(contentEl).addButton((b) => b.setButtonText("Join").setCta().onClick(async () => {
      const code = invite.trim();
      if (!code) return void new import_obsidian9.Notice("Invite code required");
      if (code.startsWith("sgd_")) {
        try {
          ...
          new import_obsidian9.Notice(`Welcome, ${me.user.display_name}!`);
          this.close();
          this.maybeOfferAi();
        } catch (e) {
          this.s.device.deviceToken = null;
          await this.s.saveDevice();
          new import_obsidian9.Notice(`Token rejected: ${e.message}`);
        }
        return;
      }
      if (!name.trim()) return void new import_obsidian9.Notice("Name and invite code required");
      try {
        ...
        await this.s.saveDevice();
        await refreshIdentity(this.s);
        new import_obsidian9.Notice(`Welcome, ${session.user.display_name}!`);
        this.close();
        this.maybeOfferAi();
      } catch (e) {
        try {
          const session = await linkDevice(this.s, invite.trim(), device);
          new import_obsidian9.Notice(`Welcome back, ${session.display_name}!`);
          this.close();
          this.maybeOfferAi();
        } catch {
          new import_obsidian9.Notice(`Could not join: ${e.message}`);
        }
      }
    })).addButton((b) => b.setButtonText("Maybe later").onClick(() => this.close()));
  }
  maybeOfferAi() {
    const m = new import_obsidian9.Modal(this.app);
    ...
    new import_obsidian9.Setting(m.contentEl).addButton((b) => b.setButtonText("Connect AI").setCta().onClick(async () => {
      m.close();
      await this.ai.beginConnect();
      new import_obsidian9.Notice("Complete the authorization in your browser \u2014 Obsidian will catch the redirect.");
...
var AiService = class {
  ...
  async completeConnect(code) {
    ...
    await this.s.saveDevice();
    new import_obsidian10.Notice("AI connected \u2713 (your own OpenRouter wallet)");
    this.s.notify();
  }
  async disconnect() {
...
var AskView = class extends import_obsidian11.ItemView {
  ...
  async renderMarkdown(el, text) {
    el.empty();
    await import_obsidian11.MarkdownRenderer.render(this.app, text, el, "", this);
  }
...
`
      ...
      new import_obsidian11.Notice("Saved to Library/AI Notes");
    } catch (e) {
      new import_obsidian11.Notice(`Could not save: ${e.message}`);
    }
  }
};
...
var ReaderView = class extends import_obsidian12.ItemView {
...
var StudyService = class {
  ...
  async saveTrail() {
    if (this.trail.length < 2) return void new import_obsidian13.Notice("Trail is empty \u2014 study a little first");
...
`
      ...
      new import_obsidian13.Notice("Trail saved to Library/Study Trails");
      this.trail = [];
    });
    dlg.open();
  }
  ...
  async bookmarkCurrent() {
    ...
    this.s.rerenderReading();
    new import_obsidian13.Notice(`Bookmarked ${f.basename}`);
  }
  // -------------------------------------------------------- flashcards
  ...
  async addFlashcard(front, back, anchor) {
    ...
    if (dup) {
      new import_obsidian13.Notice("You already have this flashcard \u{1F0CF}");
      return false;
    }
    const a = {
      annotation_id: uuid(),
    ...
    await this.s.sync.save(a);
    this.s.rerenderReading();
    new import_obsidian13.Notice("Flashcard added \u{1F0CF}");
    return true;
  }
  async review() {
    const all = await this.s.sync.allAnnotations();
    ...
    if (!due.length) return void new import_obsidian13.Notice("No cards due \u2014 well done!");
...
var NameModal = class extends import_obsidian13.Modal {
  ...
  onOpen() {
    ...
    let v = this.initial;
    new import_obsidian13.Setting(this.contentEl).setName("Name").addText((t) => t.setValue(this.initial).onChange((x) => v = x));
    new import_obsidian13.Setting(this.contentEl).addButton((b) => b.setButtonText("Save").setCta().onClick(() => {
      this.close();
...
var ReviewModal = class extends import_obsidian13.Modal {
...
init_src();
var SGSettingsTab = class extends import_obsidian14.PluginSettingTab {
  constructor(p) {
    super(p.app, p);
    this.p = p;
  }
  display() {
    ...
    if (!s.signedIn) {
      new import_obsidian14.Setting(el).setName("Join Scripture Graph").setDesc("Sign in with your family invite code").addButton((b) => b.setButtonText("Join\u2026").setCta().onClick(() => new WelcomeModal(s, this.p.ai, () => this.display()).open()));
    } else {
      new import_obsidian14.Setting(el).setName(`Signed in as ${s.device.displayName ?? "?"}`).setDesc(`Groups: ${s.groups.map((g) => g.name).join(", ") || "none yet"}`).addButton((b) => b.setButtonText("Sign out this device").onClick(async () => {
        try {
      ...
      new import_obsidian14.Setting(el).setName("Link another device").setDesc("Creates a one-time code (valid 1 hour) to sign THIS account in on your phone").addButton((b) => b.setButtonText("Create code").onClick(async () => {
        try {
          ...
          new import_obsidian14.Notice(e.message);
        }
      }));
      new import_obsidian14.Setting(el).setName("Create a group").addText((t) => t.setPlaceholder("e.g. Richins Family").then((t2) => {
        new import_obsidian14.Setting(el).addButton((b) => b.setButtonText("Create").onClick(async () => {
          const name = t2.getValue().trim();
          if (!name) return;
          await s.api.createGroup(name);
          await refreshIdentity(s);
          new import_obsidian14.Notice(`Group \u201C${name}\u201D created`);
          this.display();
        }));
      }));
      for (const g of s.groups) {
        new import_obsidian14.Setting(el).setName(`\u{1F465} ${g.name}`).setDesc(g.role).addButton((b) => b.setButtonText("Invite\u2026").onClick(async () => {
          try {
      ...
      new import_obsidian14.Setting(el).setName("Join a group").setDesc("Paste a group invite code").addText((t) => t.setPlaceholder("XXXX-XXXX-XXXX").then((t2) => {
        new import_obsidian14.Setting(el).addButton((b) => b.setButtonText("Join group").onClick(async () => {
          try {
            const r = await s.api.acceptInvite(t2.getValue().trim());
            await refreshIdentity(s);
            new import_obsidian14.Notice(`Joined ${r.group_name ?? "group"}`);
            this.display();
          } catch (e) {
            new import_obsidian14.Notice(e.message);
          }
    ...
    new import_obsidian14.Setting(el).setName("Chapter links open My Study page").setDesc("Links like [[Matthew 5]] land on your editable page (the scripture is embedded there). Verse-precise links still open the exact verse.").addToggle((t) => t.setValue(s.settings.chapterLinksToMyStudy).onChange(async (v) => {
      s.applySettings({ chapterLinksToMyStudy: v });
      await this.p.saveSharedSettings();
    }));
    new import_obsidian14.Setting(el).setName("Show AI Library folder in sidebar").setDesc("Off keeps the AI Library out of the file explorer on this device \u2014 study pages still show and link its content (read-only). Leave off on family devices.").addToggle((t) => t.setValue(s.device.showAiLibrary).onChange(async (v) => {
      s.device.showAiLibrary = v;
      document.body.toggleClass("sg-hide-ai-lib", !v);
      await s.saveDevice();
    }));
    new import_obsidian14.Setting(el).setName("Reading scene").setDesc("An ambient living backdrop behind the scriptures").addDropdown((d) => {
    ...
    new import_obsidian14.Setting(el).setName("Default for new notes/highlights").setDesc("\u{1F510} Only me (synced) is recommended; \u{1F512} device-only never uploads anywhere").addDropdown((d) => d.addOption("private", "\u{1F510} Only me (synced)").addOption("local", "\u{1F512} Only me (this device)").setValue(s.settings.defaultVisibility).onChange(async (v) => {
      s.settings.defaultVisibility = v;
      await this.p.saveSharedSettings();
    }));
    new import_obsidian14.Setting(el).setName("Show my marks").addToggle((t) => t.setValue(s.device.showScopes.mine).onChange(async (v) => {
      s.device.showScopes.mine = v;
      await s.saveDevice();
      s.notify();
    }));
    for (const g of s.groups) {
      new import_obsidian14.Setting(el).setName(`Show ${g.name}`).addToggle((t) => t.setValue(s.device.showScopes.groups[g.group_id] !== false).onChange(async (v) => {
        s.device.showScopes.groups[g.group_id] = v;
        await s.saveDevice();
        s.notify();
      }));
    }
    new import_obsidian14.Setting(el).setName("Show public highlights").addToggle((t) => t.setValue(s.device.showScopes.public).onChange(async (v) => {
    ...
    if (!s.aiConnected) {
      new import_obsidian14.Setting(el).setName("Connect AI").setDesc(
        ...
        new import_obsidian14.Notice("Finish in the browser \u2014 Obsidian catches the redirect. If it doesn't return, paste the code below.");
        this.display();
      }));
      new import_obsidian14.Setting(el).setName("Paste authorization code").setDesc("Only needed if the browser redirect didn't come back").addText((t) => t.setPlaceholder("code from openrouter.ai").then((t2) => {
        new import_obsidian14.Setting(el).addButton((b) => b.setButtonText("Finish connection").onClick(async () => {
          try {
      ...
      new import_obsidian14.Setting(el).setName("AI connected \u2713").addButton((b) => b.setButtonText("Disconnect").setWarning().onClick(async () => {
        await this.p.ai.disconnect();
        this.display();
      }));
      new import_obsidian14.Setting(el).setName("Preferred models").addDropdown((d) => d.addOption("auto", "AUTO \u2014 recommended").addOption("fast", "Fast & cheap").addOption("deep", "Deep research").addOption("best", "Highest quality").addOption("cheapest", "Cheapest").addOption("specific", "Specific model\u2026").setValue(s.device.aiTier).onChange(async (v) => {
        s.device.aiTier = v;
        await s.saveDevice();
        this.display();
      }));
      if (s.device.aiTier === "specific") {
        new import_obsidian14.Setting(el).setName("Model id").setDesc("Advanced: any OpenRouter model id").addText((t) => t.setValue(s.device.aiSpecificModel ?? "").setPlaceholder(TIER_CANDIDATES.deep[0] ?? "").onChange(async (v) => {
          s.device.aiSpecificModel = v.trim() || null;
          await s.saveDevice();
        }));
      }
      new import_obsidian14.Setting(el).setName("Monthly safety cap (USD)").setDesc("Scripture Graph stops starting AI requests past this amount").addText((t) => {
      ...
      void this.p.state.budget.state().then(async (b) => {
        ...
        new import_obsidian14.Setting(el).setName(
      ...
      new import_obsidian14.Setting(el).setName("Let AI read my private notes as context").setDesc("Off by default. AI never modifies your notes either way (\xA727).").addToggle((t) => t.setValue(s.device.aiUsePersonalNotes).onChange(async (v) => {
        s.device.aiUsePersonalNotes = v;
        await s.saveDevice();
      }));
    }
    el.createEl("h2", { text: "My data" });
    new import_obsidian14.Setting(el).setName("Export my data").setDesc("All annotations + highlights \u2192 Markdown/JSON in Library/Exports").addButton((b) => b.setButtonText("Export").onClick(() => void this.p.exportMyData()));
    new import_obsidian14.Setting(el).setName(`Plugin version: v${this.p.manifest.version}`).setDesc("Updates come straight from your family server \u2014 no sync games").addButton((b) => b.setButtonText("Check for updates").onClick(() => void this.p.checkForUpdate(false)));
    new import_obsidian14.Setting(el).setName("Debug: copy interaction log").setDesc("Copies what the touch layer saw (taps, selections, decisions) \u2014 paste it to whoever is fixing a bug").addButton((b) => b.setButtonText("Copy log").onClick(async () => {
...
` + traceDump2()
      ...
      new import_obsidian14.Notice("Interaction log copied \u2014 paste it in a message");
    ...
    new import_obsidian14.Setting(el).setName("Server address").setDesc(
  ...
  async renderAdmin(el) {
    ...
    try {
      ...
      new import_obsidian14.Setting(el).setName("New family account invite").addButton((b) => b.setButtonText("Create invite").onClick(async () => {
...
var CodeModal = class extends import_obsidian14.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian14.Setting(this.contentEl).addButton((b) => b.setButtonText("Copy").setCta().onClick(async () => {
...
async function migrateFromAnnotate(s) {
  ...
  await s.store.put(FLAG, true);
  if (count) {
    new import_obsidian15.Notice(`Scripture Graph: imported ${count} highlight${count === 1 ? "" : "s"} from the old plugin (kept device-local \u2014 share any of them from the verse popover).`);
  }
}

// src/main.ts
var WriteModal = class extends import_obsidian16.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian16.Setting(this.contentEl).addButton((b) => b.setButtonText("Save").setCta().onClick(async () => {
...
var SGPlugin = class extends import_obsidian16.Plugin {
  state;
  ai;
  ann;
  study;
  ...
  async onload() {
    ...
    this.registerObsidianProtocolHandler("scripture-graph-auth", (params) => {
      ...
      if (!code) return void new import_obsidian16.Notice("AI connection failed: no code in redirect");
      this.ai.completeConnect(code).catch((e) => new import_obsidian16.Notice(e.message));
    });
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (!hit) return void new import_obsidian16.Notice("Select some scripture text first");
        ...
        new import_obsidian16.Notice(`Highlighted ${verseDisplay(hit.verseId) ?? hit.verseId}`);
      }
    });
    this.addCommand({
      id: "note-selection",
      ...
      callback: () => {
        ...
        new NoteModal(this.state, verseDisplay(hit.verseId) ?? hit.verseId, (text) => {
          ...
          new import_obsidian16.Notice("Note saved");
        }).open();
      }
    });
    this.addCommand({
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (!sel) return void new import_obsidian16.Notice("Select the text for the card back first");
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian16.Notice("Synced");
      }
    });
    this.addCommand({
      id: "cleanup-marks",
    ...
    back.onclick = () => {
      ...
      if (f instanceof import_obsidian16.TFile) void this.app.workspace.getLeaf().openFile(f);
    };
    ...
    this.app.workspace.onLayoutReady(() => {
      void (async () => {
        ...
        if (plugins?.enabledPlugins?.has?.("scripture-graph-annotate")) {
          ...
          new import_obsidian16.Notice("Old Scripture Graph plugin retired (it kept re-enabling itself via sync)");
        }
        ...
        if (seen !== this.manifest.version) {
          ...
          new import_obsidian16.Notice(`Scripture Graph v${this.manifest.version} loaded`);
        }
  ...
  async checkForUpdate(silent) {
    ...
    try {
      const mf = await (0, import_obsidian16.requestUrl)({ url: `${base}/plugin/manifest.json`, throw: false });
      if (mf.status !== 200) {
        if (!silent) new import_obsidian16.Notice("No plugin build published on the server yet");
        return;
      }
      let manifest;
      try {
        ...
        if (!silent) new import_obsidian16.Notice("Update channel returned an unreadable manifest");
        return;
      }
      const remote = manifest.version ?? "";
      if (!newerVersion(remote, this.manifest.version)) {
        if (!silent) new import_obsidian16.Notice(`Up to date \u2014 v${this.manifest.version}`);
        return;
      }
      const [main, styles] = await Promise.all([
        (0, import_obsidian16.requestUrl)({ url: `${base}/plugin/main.js`, throw: false }),
        (0, import_obsidian16.requestUrl)({ url: `${base}/plugin/styles.css`, throw: false })
      ]);
      if (main.status !== 200 || main.text.length < 1e4) {
        if (!silent) new import_obsidian16.Notice("Update download failed \u2014 try again");
        return;
      }
      ...
      new import_obsidian16.Notice(`Scripture Graph updated to v${remote} \u2014 reloading\u2026`, 8e3);
      window.setTimeout(() => {
        const cmds = this.app.commands;
        cmds?.executeCommandById?.("app:reload");
      }, 900);
    } catch (e) {
      if (!silent) new import_obsidian16.Notice(`Update check failed: ${e.message}`);
    }
  }
  onunload() {
    this.ann.stop();
  ...
  openMyStudy(chapterTitle2) {
    ...
    if (this.app.metadataCache.getFirstLinkpathDest(companion, "")) {
      ...
      new import_obsidian16.Notice("No My Notes page exists for this chapter yet");
    }
  }
  ...
  openNavigator() {
    new SGNavigatorModal(this.app, {
      ...
      listFolder: (path) => {
        ...
        const folders = [];
        const files = [];
        if (af instanceof import_obsidian16.TFolder) {
          for (const ch of af.children) {
            if (ch instanceof import_obsidian16.TFolder) {
              folders.push({ name: ch.name, path: ch.path });
            } else if (ch instanceof import_obsidian16.TFile && ch.extension === "md" && !ch.basename.startsWith("_")) {
  ...
  pickScene() {
    ...
    const set = (value, label) => {
      ...
      new import_obsidian16.Notice(`Reading scene: ${label}`);
    };
...
${text.trim()}
`);
      new import_obsidian16.Notice("Added to your notes \u270D\uFE0F");
    }).open();
  }
  ...
  enforceReadOnly() {
    for (const leaf of this.app.workspace.getLeavesOfType("markdown")) {
      const view = leaf.view;
      if (!(view instanceof import_obsidian16.MarkdownView) || !view.file) continue;
      const path = view.file.path;
      const canonical = path.startsWith(CANONICAL_PREFIX);
      const aiLibrary = path.startsWith(LIBRARY_PREFIX);
      const mobileStudyPage = import_obsidian16.Platform.isMobile && path.startsWith(PERSONAL_PREFIX) && path.endsWith(" - My Notes.md");
      ...
      if (canonical && !this.noticedReadOnly.has(path)) {
        ...
        new import_obsidian16.Notice("Scripture is read-only \u2014 highlight it, or write in \u270F\uFE0F My Notes");
      }
    }
  }
  ...
  async exportMyData() {
    ...
    new import_obsidian16.Notice("Exported to Library/Exports");
  }
  ...
  async cleanupMarks() {
    ...
    new import_obsidian16.Notice(removed ? `Cleaned up ${removed} duplicate/junk mark${removed === 1 ? "" : "s"} \u{1F9F9}` : "Nothing to clean \u2014 your marks are tidy \u2728");
  }
  async writeExport(path, content) {
    const existing = this.app.vault.getAbstractFileByPath(path);
    if (existing instanceof import_obsidian16.TFile) await this.app.vault.modify(existing, content);
