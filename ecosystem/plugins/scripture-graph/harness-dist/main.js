...
`
  ...
  var SGLibraryView = class extends ItemView {
  ...
  window.sgLibPage = () => {
    const host = {
      openChapter: (t) => log(`lib \u2192 ${t}`),
      openNote: (l) => log(`lib \u2192 note ${l}`),
      ...
      listFolder: (path) => ({
        folders: [],
        files: [{ name: "Sample page", path: `${path}/s.md` }]
      }),
      openPath: (p) => log(`lib \u2192 open ${p}`),
      openTimeline: () => log("lib \u2192 timeline")
    };
    const view2 = new SGLibraryView({}, { app: {} }, host);
    window.sgLibView = view2;
    void view2.onOpen().then(() => {
      view2.contentEl.style.cssText = "position:fixed;inset:0;z-index:70;background:#0b0b12;overflow:auto;";
      document.body.appendChild(view2.contentEl);
    });
  };
  void redecorate();
