...
def _cmd_fetch_locked(ctx, args, fetchers, glib, ensure_registry):
    ...
    if args.what in ("church-music", "all"):
        from scripturegraph.corpus import churchmusic
        out["church_music"] = churchmusic.fetch(ctx)
...
def main(argv=None) -> int:
    ...
    sp.add_argument("what", choices=["conference", "history", "jsp", "prophets", "hymns", "footnotes", "cfm",
                                     "od", "apparatus", "collections", "glib-markdown", "music-links", "church-music", "gospel-library", "all"])
